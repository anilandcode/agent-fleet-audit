...
export function ControlPlane() {
  ...
  return <main className="app-shell">
    <aside className={navOpen ? "sidebar open" : "sidebar"} id="control-plane-navigation"><div className="sidebar-top"><Link className="brand" href="/"><BrandMark /></Link><button className="sidebar-close" type="button" onClick={() => setNavOpen(false)} aria-label="Close control plane navigation">Close</button></div><div className="workspace"><span>WORKSPACE</span><strong>{snapshot.tenant.workspace.name}</strong><small>{snapshot.tenant.workspace.role} · {snapshot.tenant.organization.plan}</small></div><nav aria-label="Control plane navigation"><a className="current" href="#overview" onClick={() => setNavOpen(false)}>Overview</a><a href="#trace" onClick={() => setNavOpen(false)}>Traces</a><a href="#policy" onClick={() => setNavOpen(false)}>Policies</a><a href="#evidence" onClick={() => setNavOpen(false)}>Evidence</a><a href="#findings" onClick={() => setNavOpen(false)}>Findings</a></nav><div className="sidebar-bottom"><span>DEMO MODE</span><p>Every event is deterministic and explicitly labeled.</p><Link href="/">← Marketing site</Link></div></aside>
    <section className="app-main" id="overview"><header className="app-header"><div className="app-heading"><button className="app-menu-toggle" type="button" aria-label="Open control plane navigation" aria-controls="control-plane-navigation" aria-expanded={navOpen} onClick={() => setNavOpen(true)}><i /><i /></button><div><span className="eyebrow">{snapshot.tenant.organization.name}</span><h1>{snapshot.tenant.fleet.name}</h1></div></div><div className="header-actions"><span className="live-pill"><i /> governed</span><button className="app-run" type="button" onClick={runVendorRiskWorkflow} disabled={runningWorkflow}>{runningWorkflow ? "Running review…" : "Run vendor review"}</button><a className="app-export" href="/api/v1/reports/demo/export">Export report ↗</a></div></header>
